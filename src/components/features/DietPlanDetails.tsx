...
import {
  ...
  MapPin, Timer, Droplets, FileText, History, ImageIcon
} from 'lucide-react';
import { imageService } from '../../services/imageService';
import { useDietStore } from '../../stores/dietStore';
import { useAuthStore } from '../../stores/authStore';
import { DietPlan, DietPlanMeal } from '../../models/Diet';
import { MealsSection } from '../common/MealsSection';
...
export function DietPlanDetails({
  ...
  useEffect(() => {
    if (activeTab === 'history') {
      ...
      loadDietPlanHistory(plan.id).finally(() => setHistoryLoading(false));
    }
  }, [activeTab, plan?.id, loadDietPlanHistory, dietPlanHistory.length]);
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <div className='flex justify-between items-center mb-4'>
                ...
                <p onClick={handleRequestChange} className='text-blue-500 hover:cursor-pointer'><span className="text-xs font-medium">Request Change</span></p>
              </div>
              {(plan.description || plan.notes) && (
                <p className="text-gray-600 mb-4">{plan.description || plan.notes}</p>
              )}
            ...
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-3">Daily Macros</h3>
        ...
        {activeTab === 'meals' && (
          <div className="bg-white rounded-sm border border-gray-200 p-6">
            <MealsSection
              ...
              showNutrition={true}
              onMealClick={(meal) => {
                // Handle meal click - could open a detailed modal
                console.log('Meal clicked:', meal);
              }}
              onSectionClick={(mealType, meals) => {
                // Handle section click - could navigate to meal type view
                console.log('Section clicked:', mealType, meals);
              }}
            />
          </div>
        )}
          ...
          <div className="bg-white rounded-sm border border-gray-200 p-6">
            ...
            {historyLoading ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">Loading plan history...</p>
              </div>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-400">
                  Changes to this diet plan will appear here
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {dietPlanHistory.map((historyItem, index) => (
                  <div key={historyItem.id} className="border border-gray-200 rounded-sm p-4">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="text-sm font-medium text-gray-900">
                            {historyItem.changer?.firstName} {historyItem.changer?.lastName}
                          </h4>
